...
struct Canvas {
    ...
    func makeShowcase(filename: String, titleSize: CGFloat, subtitleSize: CGFloat) {
        ...
        text("وش الرأي؟", x: width * 0.10, y: height * 0.51, w: width * 0.80, h: height * 0.07, size: titleSize, weight: .black, align: .center)
...
guard let source = NSImage(contentsOf: sourceURL) else {
    fatalError("Missing source image at \(sourceURL.path)")
}
