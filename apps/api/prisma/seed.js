...
async function upsertRole(code, nameAr, isSystem = true) {
...
async function upsertPermission(code, nameAr) {
...
async function main() {
    ...
    const findPerm = (code) => perms.find((p) => p.code === code);
    ...
    await Promise.all(Array.from({ length: 10 }, (_, idx) => prisma.source.upsert({
