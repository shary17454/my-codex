...
async function getVersionContext() {
  const versions = await api(`/apps/${appId}/appStoreVersions?filter[platform]=IOS&filter[appStoreState]=PREPARE_FOR_SUBMISSION&include=appStoreVersionLocalizations,build&limit=10`);
  const version = versions.data[0];
  if (!version) throw new Error('No PREPARE_FOR_SUBMISSION iOS version found.');
...
async function uploadScreenshots(localizationId) {
  ...
  for (const job of jobs) {
    ...
    const files = fs.readdirSync(job.dir).filter(name => name.endsWith('.png')).sort().slice(0, 3);
    for (const name of files) {
      const uploaded = await uploadOneScreenshot(set.id, path.join(job.dir, name));
      results.push({ displayType: job.displayType, file: name, id: uploaded.id, uploaded: uploaded.attributes.uploaded });
    }
  }
  return results;
}
