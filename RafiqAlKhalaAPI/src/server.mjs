...
function numberField(input, name, fallback) {
...
function planTrip(input) {
  ...
  const durationHours = numberField(input, 'durationHours', 8);
  const peopleCount = numberField(input, 'peopleCount', 1);
  const vehicleType = input.vehicleType || 'دفع رباعي';
  const budgetSar = numberField(input, 'budgetSar', 0);
  ...
  return {
    ...
    warnings: buildSafetyAlerts({
      temperature: numberField(input, 'temperature', 38),
      windSpeed: numberField(input, 'windSpeed', 22),
      airQualityIndex: numberField(input, 'airQualityIndex', 80),
      hasRainRisk: Boolean(input.hasRainRisk),
      networkGapKm: numberField(input, 'networkGapKm', 0)
    })
  };
}
...
async function router(req, res) {
  ...
  try {
    if (req.method === 'GET' && url.pathname === '/health') {
      return json(res, 200, { ok: true, service: 'Rafiq Al Khala API', version: '0.1.0' });
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  createServer().listen(port, host, () => {
    console.log(`Rafiq Al Khala API running on http://${host}:${port}`);
  });
}
