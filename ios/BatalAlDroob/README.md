...
- verifies `MARKETING_VERSION = 2.4`,
- verifies `CURRENT_PROJECT_VERSION >= 171`,
...
creating `2.3` because that version number had already been used. For the next
candidate, create/open App Store version `2.4` and keep Xcode Cloud > Workflow >
Next Build Number at `171` or higher. Do not reuse any uploaded build number.
...
a fresh signed Xcode Cloud build `171` or higher on release train `2.4`,
